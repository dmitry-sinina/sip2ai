...
// them verbatim instead of quoting them onto a single line. It is also used for
// the full OpenAI sideband event payloads, which are likewise multiline.
package siplog
...
// (full SIP frames and OpenAI sideband event payloads).
...
func (h *Handler) Handle(ctx context.Context, r slog.Record) error {
	// Prepend stored attrs (like callid) to the message as bare values,
...
func (h *Handler) WithAttrs(attrs []slog.Attr) slog.Handler {
	// Separate "callid" attrs (rendered as bare prefix in message) from
